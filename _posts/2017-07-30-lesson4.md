...
<p>
  Note that this entire expression \(n=\textbf{w}_1 \textbf{p}+b\) is still
  just a scalar, as \(\textbf{w}_1\) is a \(1 \times R \) matrix while \(
  \textbf{p} \) is a \( R \times 1 \) matrix which results in a \( 1 \times 1\)
