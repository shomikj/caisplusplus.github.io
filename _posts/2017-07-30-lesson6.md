...
<p>
  ...
  receptive field, or the <b>filter size</b>. The connectivity is always full along the
...
<p>
  ...
  Furthermore, each of these filters will be trained to detect some unique feature
  on the same input region. 
</p>

<p>
  <b>Stride</b> specifies how to slide a filter over the input region. This is what
...
<p>
  <b>Zero padding</b> is simply padding the input image with 0's. So a zero padding of
  3 would add 3 layers of zero valued pixels around the borders. Since applying convolutions
...
<p>
  ...
  a final prediction vector (which we need for classification and regression). A CNN outputs a
...
<ul>
  ...
  <li>
    <b>VGGNet:</b>  A visualization of the VGGNet architecture is shown below.
