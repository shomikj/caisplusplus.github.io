...
<p>
  ...
  generalize to an arbitrary network. To see a detailed derivation of these
  equations visit <a
...
<p>
  So in summary here are the derived equations of backpropagation we have
  arrived at. For the sake of simple say that the activation function is the
  same throughout the entire network. However, you should see that this could
  easily be generalized to different activation functions for each layer. Given
  a neural network with \(\textbf{L}\) layers, input \(\textbf{x}\) and an
  activation function used by all layers \( f \). The \( s^m \) term is the
  sensitivities referred to in the proof in the supplemental lesson or the
  "error" of each individual in layer \( m \) referred to as \( \delta^m
  earlier \).
  ...
  $$
  \textbf{n}^{m+1} = \textbf{W}^{m+1}\textbf{a}^{m} + \textbf{b}^{m+1}
  $$

  $$
  \textbf{a}^{m+1} = f(\textbf{n}^{m+1})
  $$
  ...
  $$
  \textbf{s}^{L} = f'(\textbf{n}^L) * \frac{\partial J}{\partial \textbf{a}}
  $$

  Remember that \(f' = \frac{\partial f}{\partial \text{n}^m} \). Then backpropogate this sensitivity to all the other layers.

  $$
  \textbf{s}^{m} = 
  f'(\textbf{n}^{m}) \left( \textbf{W}^{m + 1} \right)^{T}
  \textbf{s}^{m+1}
  $$
  For \( m = L - 1 \) to \( m = 0 \). Using the sensitivities we can now update
  the parameters of the network. \( J \) is our loss function. For iteration \(
  k + 1 \) using \( k \) using learning rate \( \alpha \). Remember that the
  learning rate is how fast we descend the gradient.
...
<p>
  As a final clarification, backpropagation is the step of propagating the sensitivities
...
<p>
  In our equations of backpropagation we have a generalized loss function \( J
  \). This cost function is what we minimized through stochastic
  gradient descent (SGD) and tells how good the model is doing given the
  parameters.
</p>

<p>
  Before we saw the mean squared error used as a loss function. However, the mean squared
  error may not always be the best cost function. In fact a more popular loss
  function is the <i>cross entropy cost function.</i> However, before we get
...
Watch the following video as an introduction to futher methods of optimization.
...
<p>
  ...
  commonly used to refer to all the parameters of our network. 
  $$
...
<p>
  ...
  any function it takes exponentially more neurons for complex representations
  with shallow network as opposed to deep networks. These deeper layers of
...
<p>
  ...
  until 2012 and it being solved is responsible for much of the deep learning boom. The cause
...
<p>
  ...
  $$
  \textbf{s}^4 = f'(\textbf{n}^4)(\textbf{W}^{5})^T \frac{\partial J}{\partial \textbf{n}^5}
  $$

  $$
  \textbf{s}^3 = f'(\textbf{n}^3)(\textbf{W}^{4})^T f'(\textbf{n}^4)(\textbf{W}^{5})^T \frac{\partial J}{\partial \textbf{n}^5}
  $$

  $$
  \textbf{s}^2 = f'(\textbf{n}^2)(\textbf{W}^{3})^T f'(\textbf{n}^3)(\textbf{W}^{4})^T f'(\textbf{n}^4)(\textbf{W}^{5})^T \frac{\partial J}{\partial \textbf{n}^5}
  $$

  $$
  \textbf{s}^1 = f'(\textbf{n}^1)(\textbf{W}^{2})^T f'(\textbf{n}^2)(\textbf{W}^{3})^T f'(\textbf{n}^3)(\textbf{W}^{4})^T f'(\textbf{n}^4)(\textbf{W}^{5})^T \frac{\partial J}{\partial \textbf{n}^5}
  $$
...
<p>
  ...
  values. The key here is that the values start small. The vanishing gradient
