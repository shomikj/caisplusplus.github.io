...
planted:
  ...
  * And a new one: $$\delta^{(i)}$$ is a vector representing the "error" of each of the individual neurons in the $$i$$th layer. Mathematically, it is given by $$\frac{\partial J}{\partial z^{(i)}}$$, where $$z^{(i)}$$ is the weighted sum of the neurons within layer in question. This tells us which way we should try to shift the weighted sum of any one neuron in the given layer in order
  to decrease our final cost.
...
<p>
  ...
  <b>Explanation: </b> Basically, what we've done here is start with the final layer's
...
<p>
  ...
  <b>Explanation:</b> Again, what we've done here is start with the current layer's
...
<h4>Calculating the Gradients</h4>
<p>
