...
<p>
  ...
  $$
  \textbf{z}^{0} = \textbf{x} \text{  (Starting with the input layer)}
  $$
...
<p>
  ...
  \textbf{D}(\textbf{z}^{m}) \left( \textbf{W}^{m + 1} \right)^{T}
  \delta^{m+1}
  $$

  In the above equation \( \textbf{D}(\textbf{z}^m) \) is the diagonalized
  matrix where the diagonal elements correspond to \( \frac{ \partial
  f(z^m_j)}{\partial z^m_j} \). 
