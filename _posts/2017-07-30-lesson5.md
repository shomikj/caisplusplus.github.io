...
<p>
  ...
  to be equal to the weight matrix \(\textbf{W}^{m + 1}\). (We apply a transpose to make the dimensions match up for the matrix multiplication.)
  ...
  to its weights. Since \(\textbf{z}^{m} = \textbf{W}^{m} a^{m-1} + \textbf{b}^{m}\), this second partial derivative evaluates to \(a^{m-1}\). Again, we apply a transpose to make the dimensions match up for the matrix multiplication.

  <br><br>

  (In the case of the first layer in the network, \(a^{m-1}\) will be the 
